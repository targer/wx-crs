package org.open.crs.service.lucene;
...
import org.open.crs.dto.lucene.TxtSearchResultDTO;
...
//            bufferedReader = Files.newBufferedReader(Paths.get(queries));
    ...
    public TxtSearchResultsDTO search(String line) throws ParseException, IOException {
        TxtSearchResultsDTO txtSearchResultsDTO = new TxtSearchResultsDTO();
...
//        doPagingSearch(bufferedReader, indexSearcher, query, hitsPerPage, raw, queries == null && queryString == null);
        int start = 0;
        for (int i = 0; i < hits.length; i++) {
            TxtSearchResultDTO txtSearchResultDTO = new TxtSearchResultDTO();
            System.out.println("doc=" + hits[i].doc + " score=" + hits[i].score);

            Document doc = indexSearcher.doc(hits[i].doc);
            String path = doc.get("path");
            if (path != null) {
                txtSearchResultDTO.setPath(path);
                System.out.println((i + 1) + ". " + path);
                String contents = doc.get("contents");
                if (contents != null) {
                    txtSearchResultDTO.setContent(contents);
                    System.out.println("   Content: " + doc.get("contents"));
                }
            } else {
                System.out.println((i + 1) + ". " + "No path for this document");
            }
            txtSearchResultsDTO.getTxtSearchResultDTOs().add(txtSearchResultDTO);
        }
